...
private:
  ...
  void OnNewItemAttached(ArkUI_NodeAdapterEvent *event) {
    ...
    FOOTSTONE_LOG(INFO) << "xxx hippy, list xx reuse, pager OnNewItemAttached, index: " << index << ", this: " << this;
  }

  // Item从可见区域移除。
  void OnItemDetached(ArkUI_NodeAdapterEvent *event) {
    FOOTSTONE_LOG(INFO) << "xxx hippy, list xx reuse, pager OnItemDetached, event: " << event << ", this: " << this;
  }
